...
if( !function_exists( 'ossy_CallAuthorBoxClass' ) ) {
    /**
     *
     ...
     *
     */
    function ossy_CallAuthorBoxClass()
    {
        $display_author_box = get_theme_mod( 'ossy_enable_author_box_blog_posts', 1 );

        if ( $display_author_box == 1 ) {
            // instantiate the class & load everything else
            ossy_Author_Box_Output::getInstance();
        }
    }
    add_action('wp_loaded', 'ossy_CallAuthorBoxClass');
}

if( !class_exists( 'ossy_Author_Box_Output' ) ) {

    /**
     * Class ossy_Author_Box_Output
     */
    class ossy_Author_Box_Output {
        ...
        protected function __construct() {
            add_action( 'ossy_single_after_content', array( $this, 'output_author_box' ), 3 );
        }
