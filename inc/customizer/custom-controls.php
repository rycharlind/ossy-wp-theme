 ...
 */
if( !class_exists( 'ossy_CF7_Custom_Control' ) ) {
    class ossy_CF7_Custom_Control extends WP_Customize_Control {
        /**
        ...
        * @return bool
        ...
        public function ossy_get_cf7_forms() {
            global $wpdb;
            ...
            if ( $this->active_callback()) {
                ...
                if ($cf7) {
                    ...
                    $contact_forms[0] = __('No contact forms found', 'ossy');
                }
            }
        ...
        public function render_content() {
            $Pixova_Lite_contact_forms = $this->ossy_get_cf7_forms();

            if ( !empty($Pixova_Lite_contact_forms ) ) { ?>
                <span class="customize-control-title"><?php echo esc_html($this->label); ?></span>
                <select <?php esc_url($this->link()); ?> style="width:100%;">
                <?php echo '<option value="default">'.__('Select your contact form', 'ossy').'</option>';
 ...
 *  Custom Control: Text
 */
if( !class_exists( 'ossy_Text_Custom_Control' ) ) {
    class ossy_Text_Custom_Control extends WP_Customize_Control {
        public function render_content() {
            $output = '';
